...
from ..data.write import write_result, delete_result, write_tileset, delete_tileset
...
class MapManager(AssetsManager):
    @servable_endpoint(path="/upload", method="POST", response_model=str)
    def upload(self, layer: dict):
        config = self.get_configuration()
        write_result(config.name, config.content_type, self.get_table(), layer, datetime.now(), description=layer.get("description", "N/A"))
        return f"Received file and processed via collect."
